...
class LaboratoryWorkflow:
    ...
    def report_writing(self):
        ...
        relevant_docs = vectorstore.similarity_search(f"Writing a research paper about: {self.research_topic}", k=3)
        ...
        report = "\n".join(solver.best_report[0][0])
        self.set_agent_attr("report", report)  # Ensure the report is stored in self.phd.report
        return False
...
def parse_yaml(yaml_file_loc):
    ...
    if 'groq-api-key' in agentlab_data: parser.groq_api_key = agentlab_data["groq-api-key"]
    if 'gemini-api-key' in agentlab_data: parser.gemini_api_key = agentlab_data["gemini-api-key"]
...
if __name__ == "__main__":
    ...
    if openrouter_api_key is not None and os.getenv('OPENROUTER_API_KEY') is None: os.environ['OPENROUTER_API_KEY'] = openrouter_api_key
    ...
    if args.groq_api_key is not None and os.getenv('GROQ_API_KEY') is None: os.environ["GROQ_API_KEY"] = args.groq_api_key
    if args.gemini_api_key is not None and os.getenv('GEMINI_API_KEY') is None: os.environ["GEMINI_API_KEY"] = args.gemini_api_key
    if not api_key and not deepseek_api_key and not groq_api_key and not gemini_api_key : raise ValueError("API key must be provided via --api-key / -deepseek-api-key or the OPENAI_API_KEY / DEEPSEEK_API_KEY environment variable.")
